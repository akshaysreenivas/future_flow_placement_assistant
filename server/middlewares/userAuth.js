...
module.exports = async (req, res, next) => {
    try {
        ...
        if (!authToken) return res.json({ login:false, status: false, message: "no auth token" });
        // decoding the token 
        const decoded = jwt.verify(authToken, process.env.JWT_KEY);
        // checking for if the user exist with the decoded id 
        const user = await userModel.findOne({ _id: decoded.id });
        if (!user) return res.json({login:false, status: false, message: "Unauthorized" });
        ...
        res.json({login:false, status: false, message: "Unauthorized" });
    }
};